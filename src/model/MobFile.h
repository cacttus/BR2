...
//Temporary storage class for reading obj file contents
...
public:
  ...
  std::shared_ptr<ArmatureNode> _pCurArmData = nullptr;
...
public:
  ...
  string_t getMobDir();
  std::vector<std::shared_ptr<ModelNode>>& getModelNodes() { return _vecModelNodes; }
...
private:
  ...
  std::vector<std::shared_ptr<ModelNode>> _vecModelNodes;
