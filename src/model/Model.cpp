...
WorldObjectSpec::~WorldObjectSpec() {
...
void WorldObjectSpec::serialize(std::shared_ptr<BinaryFile> fb) {
...
std::shared_ptr<ActionGroup> WorldObjectSpec::getAction(Hash32 actionNameHash) {
  for (std::shared_ptr<ActionGroup> ag : _vecActionGroups) {
...
std::shared_ptr<Armature> WorldObjectSpec::getArmatureById(int32_t armId) {
...
void WorldObjectSpec::postMobConversion() {
...
std::shared_ptr<BoneSpec> WorldObjectSpec::getBoneByArmJointOffset(int32_t ijo) {
  int32_t ij = 0;
...
//    std::shared_ptr<WorldObjectSpec> ms = Gu::getModelCache()->getOrLoadModel(_strMobName, true);
//    _pModelSpec = ms;
//  }
//  return _pModelSpec;
//}
...
WorldObject::WorldObject(std::shared_ptr<WorldObjectSpec> pModelSpec) : PhysicsNode(pModelSpec) {
}
...
void WorldObject::init() {
  PhysicsNode::init();
  //Create armatures
  for (std::shared_ptr<Armature> pArmSpec : getData<WorldObjectSpec>()->getArmatures()) {
  ...
  //Create meshes
  for (std::shared_ptr<MeshSpec> pMeshSpec : getData<WorldObjectSpec>()->getMeshes()) {
    std::shared_ptr<MeshNode> pMeshnode = MeshNode::create(pMeshSpec, getThis<WorldObject>());
...
void WorldObject::buildNodeParents() {
...
void WorldObject::addNodeToCache(std::shared_ptr<SceneNode> bn) {
...
std::shared_ptr<SceneNode> WorldObject::getNodeByName(string_t name) {
...
void WorldObject::update(float delta, std::map<Hash32, std::shared_ptr<Animator>>& mapAnimators) {
  Perf::pushPerf();
...
void WorldObject::drawForward(RenderParams& rp) {
...
std::shared_ptr<Animator> WorldObject::getAnimator(string_t actName) {
  Hash32 anh = STRHASH(actName);
  std::map<Hash32, std::shared_ptr<Animator>>::iterator it = _mapAnimators.find(anh);
  if (it != _mapAnimators.end()) {
    return it->second;
...
void WorldObject::stopAllActions() {
  for (std::pair<Hash32, std::shared_ptr<Animator>> p : _mapAnimators) {
    p.second->stop();
    //DEL_MEM(p.second);
  }
...
void WorldObject::stopAction(string_t actName) {
  Hash32 anh = STRHASH(actName);
  std::map<Hash32, std::shared_ptr<Animator>>::iterator it = _mapAnimators.find(anh);
  if (it != _mapAnimators.end()) {
...
void WorldObject::calcBoundBox(Box3f& __out_ pBox, const vec3& obPos, float extra_pad) {
  pBox.genResetLimits();
