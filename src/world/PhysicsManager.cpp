...
//  MpFloat awXZ, float awXZInc, MpFloat awY, float awYInc, MpInt mpNodesY, uint32_t iGridCountLimit) {
//  _pScene = ps;
//  _vUp = vUp;
...
//  _mpNodesY = mpNodesY;
//  _pAwareness = std::make_shared<PhysicsGridAwareness>(getThis<PhysicsManager>(), awXZ, awXZInc, awY, awYInc);
...
//}
//ivec3 PhysicsManager::v3Toi3Node(vec3& v) {
//  return v3Toi3Any(v, 1.0f / _fNodeWidth, 1.0f / _fNodeHeight);
//}
//std::multimap<float, std::shared_ptr<PhysicsGrid>>& PhysicsManager::getVisibleGrids() {
//  return _pRenderBucket->getGrids();
//}
//std::multimap<float, std::shared_ptr<SceneNode>>& PhysicsManager::getVisibleNodes() {
//  return _pRenderBucket->getObjs();
//}
//ivec3 PhysicsManager::v3Toi3Any(vec3& v, float w1, float h1) {
//  ivec3 ret;
//  float ex, ey, ez;
//
...
//  return ret;
//}
//std::shared_ptr<PhysicsGrid> PhysicsManager::getNodeAtPos(ivec3& pos) {
...
//  return ite->second;
//}
//void PhysicsManager::addGrid(std::shared_ptr<PhysicsGrid> pGrid, const ivec3& cv) {
...
//  updateWorldBox();
//}
//void PhysicsManager::updateWorldBox() {
...
//  }
//}
//void PhysicsManager::debugMakeSureNoDupes(const ivec3& vv) {
//
//  for (GridMap::iterator it = _gridMap.begin(); it != _gridMap.end(); it++) {
//    ivec3* v = it->first;
//    if (v->x == vv.x && v->y == vv.y && v->z == vv.z) {
//      Br2ThrowException("Duplicate node found.");
//    }
//  }
//
//}
//void PhysicsManager::unloadWorld() {
...
//  _gridMap.clear();
//}
//void PhysicsManager::reparentObjectByCustomBox(std::shared_ptr<PhysicsNode> ob, Box3f* pBox) {
//  bool bSet = false;
//  bool bSet2 = false;
//  //clear
...
//
//}
//void PhysicsManager::calcGridManifold(std::shared_ptr<PhysicsGrid> pGrid) {
...
//  }
//}
//void PhysicsManager::addNodeToGrid(std::shared_ptr<PhysicsNode> ob, std::shared_ptr<PhysicsGrid> pGrid) {
//  pGrid->getManifold()->add(ob);
//  ob->getManifold()->addGrid(pGrid);
//}
//void PhysicsManager::clearObjectManifoldAndRemoveFromGrid(std::shared_ptr<PhysicsNode> ob) {
//  //Clear the manifold.
...
//}
//void PhysicsManager::refreshCache() {
...
//  
//}
//bool PhysicsManager::tryRemoveObj(std::shared_ptr<PhysicsNode> ob) {
//  if (ob == nullptr) {
//    Br2LogError("Tried to remove a null object.");
//    return false;
//  }
...
//  return false;
//}
//void PhysicsManager::addObj(std::shared_ptr<PhysicsNode> ob, bool bActivate, bool bRefreshCache) {
//  if (ob->detachFromParent() == true) {
//    Br2LogWarn(ob->getName() + "Added a parented OBJ - you may get invaid results.");
//    Gu::debugBreak();
//  }
...
//  ObjMap::iterator it = _mapObjects.find(ob->getId());
//  if (it != _mapObjects.end()) {
//    Br2LogError("ERROR - Found duplicate grid object in world.");
//    Gu::debugBreak();
//  }
//  else {
...
//  }
//}
//void PhysicsManager::activate(std::shared_ptr<PhysicsNode> ob) {
//  if (ob->getIsStatic() == false) {
//    _vecActiveFrame.insert(ob);
//    ob->setIsAtRest(false);
//  }
//}
//std::shared_ptr<PhysicsNode> PhysicsManager::getObj(NodeId id) {
...
//  Perf::pushPerf();
//  std::shared_ptr<CameraNode> cam = this->_pScene->getActiveCamera();
//  for (std::pair<NodeId, std::shared_ptr<PhysicsNode>> p : _mapObjects) {
//    //Still update hidden objects (i.e., correct boundbox computation), but don't draw them.
//    p.second->update(delta, cam, std::map<Hash32, std::shared_ptr<Animator>>());
//  }
//
...
//  Perf::popPerf();
//}
//void PhysicsManager::collisionLoopDual(float delta) {
//  //Begin Dual Step
//  vec3 vGrav = _vUp * getGravityAmount();
//  //  Physics* that = this;
//  int32_t nIterV = 0, nIterA = 0;
//  uint64_t frameId = getContext()->getFpsMeter()->getFrameNumber();
...
//
//}
//void PhysicsManager::postCollide(uint64_t frameId, bool bAccel) {
//
...
//  }
//}
//bool PhysicsManager::checkMoved(vec3& p_last, vec3& p_cur, vec3& v_cur) {
...
//  collide_loop();
//}
//void PhysicsManager::unstick_loop() {
//  //Unstick everything.
...
//}
//void PhysicsManager::collide_loop() {
...
//}
//void PhysicsManager::sortObjects_CalculateSpeedboxes_And_CollectManifolds() {
...
//  }
//}
//void PhysicsManager::calc_obj_manifold(std::shared_ptr<PhysicsNode> ob) {
...
//    Br2LogWarnCycle("Bound Box Of Object " + ob->getName() + " was too large, >20000, check meshes to ensure box is accurate.");
//  }
//
//  //Update Speedbox
//  ob->calcSpeedBox();
...
//  ob->validateSanePhysics();
//}
//int32_t PhysicsManager::unstick() {
...
//  return nStuck;
//}
//bool PhysicsManager::unstick_objs(std::shared_ptr<PhysicsNode> objA, std::shared_ptr<PhysicsNode> objB, Box3f* in_bA, Box3f* in_bB) {
...
//  return false;
//}
//void PhysicsManager::unstick_if_moved(std::shared_ptr<PhysicsNode> obA, std::shared_ptr<PhysicsNode> obB, Box3f* in_bA, Box3f* in_bB,
...
//
//}
//void PhysicsManager::unstick_ob_v2(vec3& ob_in_p, vec3& ob_in_v, Box3f* boxA, Box3f* boxB,
...
//  }
//}
//int32_t PhysicsManager::collect_and_resolve_collisions() {
...
//  return numCollided;
//}
//int32_t PhysicsManager::collisionTestBox(std::shared_ptr<PhysicsNode> objA, std::shared_ptr<PhysicsNode> objB, std::multimap<float, BoxCollision>& vecBoxCollisions) {
...
//
//}
//int32_t PhysicsManager::stabalizeCollisionSystem(std::multimap<float, BoxCollision>& vecBoxCollisions) {
//  // Staballize
//  int nResolved = 0;
...
//  return nResolved;
//}
//void PhysicsManager::resolve_pair_t(BoxCollision& pCol) {
//  if (pCol._bStuck) {
//    int nnn = 0;
//    nnn++;
//  }
...
//}
//void PhysicsManager::bb_resolve_setup_data(std::shared_ptr<PhysicsNode> obA, std::shared_ptr<PhysicsNode> obB,
...
//
//}
//void PhysicsManager::bb_move_and_slide(const vec3& ob_last_p, const vec3& ob_last_v,
...
//}
//bool PhysicsManager::collideMesh(std::shared_ptr<PhysicsNode> ob, vec3* verts, size_t vOffBytes, size_t vStrideBytes, size_t vCount, v_index32* inds, int32_t iOff, int32_t iCount) {
//  bool bRet = false;
...
//  return bRet;
//}
//void PhysicsManager::refreshObjectManifold(std::shared_ptr<PhysicsNode> ob) {
//  calc_obj_manifold(ob);
//
...
//
//}
//void PhysicsManager::limitVelocity(vec3& __inout_ v) {
...
//  }
//}
//void PhysicsManager::limitAccelleration(vec3& __inout_ va) {
...
//  }
//}
//void PhysicsManager::makeGrid() {
//  //This is the main grid creation routine.
...
//  vecGen.resize(0);
//}
//void PhysicsManager::sweepGridBox(std::function<void(ivec3&)> func, ivec3& viMin, ivec3& viMax) {
...
//  if (dbg_nGrids > 5000) {
//    Br2LogWarnCycle("sweepGridBox, too many grids: " + dbg_nGrids);
//    static int n = 0;
//    if (n == 0) {
//      Gu::debugBreak();
//    }
...
//  }
//}
//void PhysicsManager::getNodeBoxForGridPos(const ivec3& pt, Box3f& __out_ box) const {
...
//}
//
//void PhysicsManager::makeOrCollectGridForPos(ivec3& cv, std::vector<std::shared_ptr<PhysicsGrid>>& vecGenerated) {
...
//
//}
//std::shared_ptr<PhysicsGrid> PhysicsManager::getNodeForPoint(vec3& pt) {
//  ivec3 iv;
...
//  return gn;
//}
//vec3 PhysicsManager::i3tov3Node(const ivec3& iNode) {
//  vec3 vOut;
...
//  }
//}
//void PhysicsManager::sweepGridFrustum_r(std::function<void(ivec3&)> func, std::shared_ptr<FrustumBase> pFrust, float fMaxDist2,
...
////    }
////}
//std::shared_ptr<PhysicsNode> PhysicsManager::findNode(string_t specName) {
