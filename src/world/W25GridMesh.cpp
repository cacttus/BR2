...
namespace BR2 {
#pragma region W25GridMesh_Internal
...
bool W25GridMesh_Internal::getMustRedoWholeMesh() {
...
void W25GridMesh_Internal::makeMeshForCell(WorldCell* gc) {
...
void W25GridMesh_Internal::makeMeshImmediately_r(BlockNode* pParent) {
...
#pragma region W25GridMesh
W25GridMesh::W25GridMesh(std::shared_ptr<WorldGrid> pGrid, GridMeshLayer::e eMatter) {
  _pint->_eMatterMode = eMatter;
  _pint->_pGrid = pGrid;
}
W25GridMesh::~W25GridMesh() {
  _pint->_pMesh = nullptr;
  _pint->_verts.clear();
  _pint->_indexes.clear();
}
std::shared_ptr<MeshNode> W25GridMesh::getMesh() { return _pint->_pMesh; }
void W25GridMesh::setVertexPreallocCount(size_t s) { _pint->_iMaxVertexCount = s; }
std::vector<W25MeshVert>& W25GridMesh::getVerts() { return _pint->_verts; }
std::vector<v_index32 >& W25GridMesh::getIndexes() { return _pint->_indexes; }
void W25GridMesh::updateTopology() {
  std::lock_guard<std::mutex> guard(_pint->_mtTopoMtx);

  if (_pint->_setTopoMod.size() > 0) {
    t_timeval tv = Gu::getMicroSeconds();
    {
      if (_pint->getMustRedoWholeMesh() == true) {
        makeMeshImmediately(false);
      }
      else {
        std::shared_ptr<W25MeshMaker> pMeshMaker = _pint->_pGrid->getWorld25()->getMeshMaker();
        std::shared_ptr<Atlas> pAtlas = _pint->_pGrid->getWorld25()->getWorldAtlas();//dynamic_cast<std::shared_ptr<Atlas>>(_pWorld25->getGlContext()->getTexCache()->getOrLoad(Utils::WorldAtlasName, true));
        AssertOrThrow2(pAtlas != nullptr);//Atlas must be loaded.
        AssertOrThrow2(pMeshMaker != nullptr);//Atlas must be loaded.
        float fWidth = BottleUtils::getCellWidth();
        float fHeight = BottleUtils::getCellHeight();
        //std::set<WorldCell*>& setCells = _pGrid->getCells();
        std::shared_ptr<SpriteBucket> pBucket = _pint->_pGrid->getWorld25()->getSpriteBucket();//dynamic_cast<std::shared_ptr<Atlas>>(_pWorld25->getGlContext()->getTexCache()->getOrLoad(Utils::WorldAtlasName, true));

        for (WorldCell* gc : _pint->_setTopoMod) {
   ...
   //       if(gc->getGeom(_eMatterMode) != W25GEOM_EMPTY) {
          pMeshMaker->redoMeshForCellNoGpuCopy(
            getThis<W25GridMesh>(), pAtlas, gc, pBucket, _pint->_eMatterMode,
            fWidth, fHeight,
            _pint->_pGrid->getWorld25()->getConfig(),
            _pint->_pGrid,
            false
          );
          //       }
...
#ifdef _DEBUG
    BRLogDebug(_pint->_pGrid->getGridPos().toString() + " -> Redo " + _pint->_setTopoMod.size() +
      ((_pint->_setTopoMod.size() == 1 && (*_pint->_setTopoMod.begin() == nullptr)) ? " grids " : " cells: ") + (Gu::getMicroSeconds() - tv) / 1000 + "ms");
#endif
    _pint->_setTopoMod.clear();
  }
}
void W25GridMesh::sendMeshToGpu() {
  BottleUtils::makeWorldMesh(_pint->_pMesh, &_pint->_verts, &_pint->_indexes);
}
...
void W25GridMesh::updateRedoMesh() {
  ...
  _pint->_setTopoMod.clear();
  _pint->_setTopoMod.insert(nullptr);
}

void W25GridMesh::updateRedoMeshForCell(WorldCell* pc, bool bNeighbors) {
  std::lock_guard<std::mutex> guard(_pint->_mtTopoMtx);

  AssertOrThrow2(pc->getGrid() == _pint->_pGrid);
  if (_pint->getMustRedoWholeMesh() == false) {
    ...
    _pint->_setTopoMod.insert(pc);
  }
}

void W25GridMesh::makeMeshImmediately(bool bAsync) {
  //This doesn't seem to make it faster.
  _pint->_verts.resize(0);
  _pint->_indexes.resize(0);

  if (_pint->_iMaxVertexCount > 0) {
    _pint->makeMeshImmediately_r(_pint->_pGrid->getRoot());
