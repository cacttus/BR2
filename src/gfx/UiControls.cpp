...
void UiElement::doMouseEvents(std::shared_ptr<InputManager> pInputManager) {
  //This seems like the moste fficient way to do this.
  ButtonState::e eLmb = pInputManager->getLmbState();
  ButtonState::e eRmb = pInputManager->getRmbState();
  ButtonState::e eMmb = pInputManager->getMmbState();
...
void UiElement::update(std::shared_ptr<InputManager> pInputManager) {
...
bool UiElement::pick(std::shared_ptr<InputManager> InputManager) {
  bool bPicked = false;
...
bool UiImage::pick(std::shared_ptr<InputManager> InputManager) {
  if (getLayoutVisible()) {
    if (getRenderVisible()) {
      if (_iPickId > 0) {
...
void UiScreen::update(std::shared_ptr<InputManager> pInputManager) {
 ...
 // if (Gu::getFpsMeter()->frameMod(2)) {
  ...
  _bIsPicked = pick(pInputManager);
}
void UiScreen::updateLayout(std::shared_ptr<InputManager> pInputManager) {
  _pCursor->left() = pInputManager->getMousePos().x;
  _pCursor->top() = pInputManager->getMousePos().y;
