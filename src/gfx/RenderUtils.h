...
public:
  ...
  static void drawWireSphereShader(std::shared_ptr<GLContext> context, float fRadius, vec4& vColor, int32_t nSlices = 5, int32_t nStacks = 5, mat4* pMatrix = nullptr);
  static void drawWireBoxShader(std::shared_ptr<GLContext> context, Box3f* box, vec3& vOffset, vec4& vColor);
  static void drawSolidBoxShaded(std::shared_ptr<GLContext> context, Box3f* box, vec3& vOffset, vec4& vColor);
  //static void drawPickBox(Box3f* box, uint32_t uiColorId);
  static void drawAxisShader(std::shared_ptr<GLContext> context, float scale = 10.0f, float lineWidth = 2.0f, mat4& transform = mat4::identity()); // Renders an Axis at the origin.
  static void drawFrustumShader(std::shared_ptr<GLContext> context, std::shared_ptr<FrustumBase> pf, vec4& avColor);//idk why there are 2
  static void drawGridShader(std::shared_ptr<GLContext> context, float r = 1.0f, float g = 1.0f, float b = 1.0f, int32_t nSlices = 60,
