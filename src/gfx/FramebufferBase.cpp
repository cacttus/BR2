...
std::shared_ptr<BufferRenderTarget> FramebufferBase::createTarget(std::shared_ptr<GLContext> context, string_t strName, GLenum internalFormat, GLenum texFormat,
  ...
  if (iIndex > maxAttach) {
    Br2ThrowException("GPU Does not support enough color attachments, wanted: " + iIndex + " max supported: " + maxAttach);
  }

  FramebufferBase::makeRenderTexture(context,&inf->_iGlTexId,
...
std::shared_ptr<BufferRenderTarget> FramebufferBase::createDepthTarget(std::shared_ptr<GLContext> context, string_t strName, int32_t w, int32_t h, int iIndex, bool bMsaaEnabled, int32_t nMsaaSamples) {
  ...
  RenderUtils::createDepthTexture(context, &inf->_iGlTexId, w, h, bMsaaEnabled, nMsaaSamples, GL_DEPTH_COMPONENT32F);
...
//}
void FramebufferBase::makeRenderTexture(std::shared_ptr<GLContext> context, GLuint* iTexId, GLenum eAttachment, GLenum eInternalFormat
  ...
  if (bMultisample) {
    ...
    //    CheckGpuErrorsDbg();
    //}
    context->glTexImage2DMultisample(GL_TEXTURE_2D_MULTISAMPLE, nSamples, eInternalFormat, iWidth, iHeight, GL_TRUE);
    context->chkErrRt();
