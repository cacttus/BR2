...
public:
    ...
    static std::shared_ptr<UiTex> create(std::shared_ptr<UIManager> gui, std::shared_ptr<Img32> generated);
    static std::shared_ptr<UiTex> create(std::shared_ptr<UIManager> gui, std::string file, bool bLoadNow = false);
...
public:
    ...
    static std::shared_ptr<Ui3Tex> create(std::shared_ptr<UIManager> gui, std::string file);
    virtual bool validate() override {
        for (int i = 0; i < 3; ++i) {
...
public:
    ...
    static std::shared_ptr<Ui9Tex> create(std::shared_ptr<UIManager> gui, std::string file);
    virtual bool validate() override {
        for (int i = 0; i < 3; ++i) {
            if (setAndValid(_images[i]) == false) {
                return false;
...
public:
    ...
    static std::shared_ptr<UiLabelSkin> create(std::shared_ptr<UIManager> gui, std::string name, uDim height = "100%");//height is also the text height - fits to the label
    UiLabelSkin() { }
