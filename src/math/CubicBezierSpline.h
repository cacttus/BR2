...
public:
  ...
  virtual void iterate(size_t pointA, size_t pointB, std::function<void(const vec3 & cur, const vec3 & last)>, double integral_t = 1.0 / 100.0) override;
  virtual void setPoints(std::vector<vec3>& v, bool tangentialize = true, bool normalize = false) override;
  virtual void eval(double t, vec3& __out_ pt) override;
  size_t numSegments();
