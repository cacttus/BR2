...
{
    if (mapLairs.size() == 0) {
        BroThrowException("There were no lairs.");
    }
    if (mapWalkers.size() == 0) {
        BroThrowException("There were no Walkers.");
    }
    ...
    if (_pDefaultLair == nullptr) {
        BroThrowException("Default lair (LA_DEFAULT) was not set..");
    }
...
t_string WorldMaker::makeGridFileName(t_string strGameName, t_string strWorldName, int32_t ix, int32_t iy, int32_t iz) {
...
std::shared_ptr<WorldGrid> WorldMaker::loadGrid(ivec3(v)) {
    ...
    t_string strFileName = makeGridFileName(_pWorld25->getRoom()->getGameName(), _pWorld25->getWorldName(), v.x, v.y, v.z);
...
void WorldMaker::saveGrid(std::shared_ptr<WorldGrid> g) {
    //BroLogDebugBeg("Saving..");
    //t_timeval tv = Gu::getMicroSeconds();
    //{
    t_string st = makeGridFileName(
