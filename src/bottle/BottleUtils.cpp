...
void BottleUtils::addTileQuad2(World25Plane* pPlane, std::shared_ptr<Atlas> pAtlas,
    ...
    if (qbm->isFull()) {
        if (bLoggedFull == false) {
            BroLogWarn("Quad buffer is full, some items will not be drawn.");
            bLoggedFull = true;
...
void BottleUtils::makeWorldMesh(std::shared_ptr<GLContext> ct, std::shared_ptr<MeshNode>& __out_ pMesh,
