...
//
//namespace BR2 {
//GodCam::GodCam(std::shared_ptr<GLContext> ct, std::shared_ptr<RenderViewport> pv) : _pContext(ct) {
...
//  //DEL_MEM(_pCamera);
//}
//void GodCam::update(std::shared_ptr<InputManager> pFingers, float dt) {
//  updateRotateAndZoom(dt);
//  updateTouches(pFingers, dt);
//  _pCamera->update(dt, std::map<Hash32, std::shared_ptr<Animator>>());
//}
//void GodCam::updateTouches(std::shared_ptr<InputManager> pFingers, float dt) {
...
//  doZoom(amt);
//}
//void GodCam::moveCameraWSAD(std::shared_ptr<InputManager> pFingers, float delta) {
//  //**Camera
