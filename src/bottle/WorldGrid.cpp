...
void WorldGrid::getCellData(BlockNode* parent, WorldCellFile* __out pFile) {
    ...
    if (parent->getIsLeaf()) {
        ...
        if (parent->getChildren() != nullptr) {
            ...
            BroThrowException("Grid was not expanded before saving.");
        }
    }
}
...
void WorldGrid::expand() {
    ...
    //BroLogDebugBeg("Dividing ", _viPos.toString(), " ..");
...
void WorldGrid::fillObjects() {
    ...
    //        BroLogError("Couldn't find solid base tile, Index:", pData->_iBaseTileIdx);
    //        Gu::debugBreak();
    //    }
    //    if(pLiquid == nullptr){
    //        //We couldn't find the texture.  You probably renamed it or deleted
    ...
    //        BroLogError("Couldn't find liquid base tile, Index:", pData->_iLiquidTileIdx);
    //            Gu::debugBreak();
    //    }
...
void WorldGrid::getObjData(WorldCellFile* pFile) {
    ...
    for (std::shared_ptr<PhysicsNode> ob : *(getManifold()->getAll())) {
        ...
        if (ob != nullptr && ob->getSpec() != nullptr && ob->getSpec()->getNameHashed() > 0) {
            ...
            BroLogError("Error, object was null or not found while saving glob");
            Gu::debugBreak();
            obd->_iType = 0;
...
void WorldGrid::beginGenAsync(std::function<void()> func) {
    ...
    _genFuture = std::async(std::launch::async, [that, func] {
        try {
            ...
            //Thread was killed
            if (that->getKilled()) {
                BroLogInfo("Grid generation cancelled.");
            }
            else {
                BroLogInfo("During Grid generation, There was an error somewhere.");
            }
...
t_string WorldGrid::getGenProfileString() {
    int iSizeKb;
...
//
//    if (obs.size() > 0) {
//        BroLogInfo("   Deleted ", obs.size(), " objects, ",(int) (Gu::getMicroSeconds() - tv) / 1000, "ms");
//    }
//
//}
    ...
    if (bUnlink) {
        ...
        if (b1->getNeighbor(nrp) != nullptr) {
            if (b1->getNeighbor(nrp) != b2) {
                Gu::debugBreak();
                BroLogError("1 Tried to link block when block already had link.");
            }
        }
        else {
        ...
        if (b2->getNeighbor(opp) != nullptr) {
            if (b2->getNeighbor(PhysicsGrid::getOppNeighborIndex(nrp)) != b1) {
                Gu::debugBreak();
                BroLogError("2 Tried to link block when block already had link.");
            }
        }
        else {
