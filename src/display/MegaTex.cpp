...
void MtFont::createFont() {
    _iBakedCharSizePixels = Gu::getContext()->getConfig()->getBakedCharSize();;
    ...
    if (!stbtt_PackBegin(&context, atlasData.get(), _atlasWidth, _atlasHeight, 0, 1, nullptr)) {
        BroLogError("Failed to initialize font");
        Gu::debugBreak();
        return;
    }
    ...
    if (!stbtt_PackFontRange(&context, (unsigned char*)_pFontBuffer->getData().ptr(), 0, _iBakedCharSizePixels, _firstChar, _charCount, _charInfo.get())) {
        BroLogError("Failed to pack font");
        Gu::debugBreak();
        return;
    }
...
void MtFont::getCharQuad(int32_t cCode, int32_t cCodePrev, FontSize fontSize, float& outWidth, float& outHeight, Box2f& texs,
    ...
    if(_bInitialized == false){
        BroLogError("Font was not initialized.");
        Gu::debugBreak();
        return;
    }
    ...
    if (getTexs().size() == 0) {
        ...
        BroLogError("Failure to save font image somewhere.");
        Gu::debugBreak();
        return;
    }
    ...
    //Debug - Save texture
    if(false){
        RenderUtils::saveTexture("./data/cache/saved_TEST.png", Gu::getContext()->getGui()->getTex()->getGlId(), GL_TEXTURE_2D);
    }
...
std::shared_ptr<MtTexPatch> MegaTex::getTex(std::shared_ptr<Img32> tx) {
    ...
    if(p!=nullptr && p->getTexs().size()>0){
        ...
        BroLogError("Failed to add texture 23458242");
        Gu::debugBreak();
    }
...
std::shared_ptr<Img32> MegaTex::compile() {
    ...
    //Now finally create
    BroLogInfo("Creating GPU Texture.");
