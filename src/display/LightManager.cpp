    ...
    _pContext(tc)
{
    ...
    if (_pContext->getConfig()->getColorSpace() == ColorSpace::e::SRGB)
    {
...
void LightManager::updateRenderData() {
    if(Gu::getContext()->getFingers()->keyPressOrDown(SDL_SCANCODE_N)){
        _pGpuDeferredParams->_fFocalDepth -= 0.5f;
    }
    if (Gu::getContext()->getFingers()->keyPressOrDown(SDL_SCANCODE_M)) {
        _pGpuDeferredParams->_fFocalDepth += 0.5f;
    }
    if (Gu::getContext()->getFingers()->keyPressOrDown(SDL_SCANCODE_J)) {
        _pGpuDeferredParams->_fFocalRange -= 0.01f;
    }
    if (Gu::getContext()->getFingers()->keyPressOrDown(SDL_SCANCODE_K)) {
        _pGpuDeferredParams->_fFocalRange += 0.01f;
    }

    if (Gu::getContext()->getFingers()->keyPressOrDown(SDL_SCANCODE_I)) {
        _pGpuDeferredParams->_fHdrToneMapExp -= 0.01f;
    }
    if (Gu::getContext()->getFingers()->keyPressOrDown(SDL_SCANCODE_O)) {
        _pGpuDeferredParams->_fHdrToneMapExp += 0.01f;
    }
    if (Gu::getContext()->getFingers()->keyPressOrDown(SDL_SCANCODE_T)) {
        _pGpuDeferredParams->_fHdrSampleExp -= 0.01f;
    }
    if (Gu::getContext()->getFingers()->keyPressOrDown(SDL_SCANCODE_Y)) {
        _pGpuDeferredParams->_fHdrSampleExp += 0.01f;
    }
...
void LightManager::setupLights(std::shared_ptr<ShadowBox> pf, std::shared_ptr<ShadowFrustum> mf) {
    Gu::pushPerf();
    int32_t nMaxPointLights = _pContext->getConfig()->getMaxPointLights();//shadowmapmaxinfluences
    int32_t nMaxDirLights = _pContext->getConfig()->getMaxDirLights();//shadowmapmaxinfluences
    ...
    for(std::pair<float, std::shared_ptr<LightNodePoint>> p : Gu::getContext()->getPhysicsWorld()->getRenderBucket()->getPointLights()){
    ...
    for (std::pair<float,std::shared_ptr<LightNodeDir>> p : Gu::getContext()->getPhysicsWorld()->getRenderBucket()->getDirLights()) {
