...
void ShadowBoxSide::updateView() {
    ...
    if (Gu::getContext()->getRenderSettings()->getDebug()->getShadowHelpVisible()) {
    ...
    if (_pLightSource->getLightRadius() < fNear) {
        fFar = Gu::getContext()->getCamera()->getFrustum()->getZFar();
    }
    else {
...
void ShadowBoxSide::collect() {
    ...
    if (Gu::getContext()->getPhysicsWorld() == nullptr) {
        return;
    }
    ...
    Gu::getContext()->getPhysicsWorld()->collectVisibleNodes(&p);
...
void ShadowBoxSide::renderShadows(std::shared_ptr<ShadowBox> pMasterBox, bool bForce) {
    ...
    pMasterBox->beginRenderSide(_eSide);
    {
        _pVisibleSet->sortAndDrawMeshes(
            ...
            return Gu::getContext()->getShaderMaker()->getShadowShader(fmt);
        },
        ...
        for (auto p : _pVisibleSet->getGrids()) {
            ...
            if (mn != nullptr) {
                if (mn->getMeshSpec() != nullptr) {
                    ...
                    sb = Gu::getContext()->getShaderMaker()->getShadowShader(fmt);
                    if(sb!=nullptr){
                        break;
        ...
        if(sb != nullptr){
            ...
            for (auto p : _pVisibleSet->getGrids()) {
                ...
                if (mn != nullptr) {
                    if (mn->getMeshSpec() != nullptr) {
                        std::shared_ptr<VertexFormat> fmt = mn->getMeshSpec()->getVertexFormat();
                        std::shared_ptr<ShaderBase> sb = Gu::getContext()->getShaderMaker()->getShadowShader(fmt);
                        if (sb != nullptr) {
                            RenderParams rp;
                            rp.setShader(sb);
                            mn->drawShadow(rp);
