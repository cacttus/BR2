...
void Atlas::compilePrecompiled(bool bMipmaps){
    ...
    if(bi->getWidth() % _vSpriteSize.x  != 0) { BroLogWarn("Atlas sprites are not evenly spaced along the x axis."); }
    if(bi->getHeight() % _vSpriteSize.y != 0){ BroLogWarn("Atlas sprites are not evenly spaced along the y axis."); }
...
void Atlas::finishCompile(std::shared_ptr<Img32> sp, bool bMipmaps){
    ...
    _pContext->getTexCache()->add(getName(), shared_from_this());
    Gu::freeImage(sp);
}
