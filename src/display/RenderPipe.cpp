...
#include "../display/ForwardFramebuffer.h"
...
void RenderPipe::init(int32_t iWidth, int32_t iHeight, t_string strEnvTexturePath) {
    ...
    if(_pDeferredShader == nullptr) {
        _pDeferredShader = Gu::getContext()->getShaderMaker()->makeShader( std::vector<t_string>{
            "d_v3x2_lighting.vs", "d_v3x2_lighting.ps" });
    }
    if(_pForwardShader == nullptr) {
        _pForwardShader = Gu::getContext()->getShaderMaker()->makeShader(std::vector<t_string>{
            "f_v3x2_fbo.vs", "f_v3x2_fbo.ps"});
    }
    Gu::getContext()->glBindRenderbuffer(GL_RENDERBUFFER, 0);
    Gu::getContext()->chkErrDbg();
    Gu::getContext()->glBindFramebuffer(GL_READ_FRAMEBUFFER, 0);
    Gu::getContext()->chkErrDbg();
    ...
    _pBlittedDeferred = std::make_shared<DeferredFramebuffer>(Gu::getContext(), iWidth, iHeight, false, 0, _vClear);
    _pBlittedDeferred->init(iWidth, iHeight, _pBlittedDepth, _pPick);
    
    _pBlittedForward = std::make_shared<ForwardFramebuffer>(Gu::getContext(), iWidth, iHeight, false, 0, _vClear);
    ...
    if (_bMsaaEnabled == true) {
        ...
        _pMsaaDeferred = std::make_shared<DeferredFramebuffer>(Gu::getContext(), iWidth, iHeight, _bMsaaEnabled, _nMsaaSamples, _vClear);
        _pMsaaDeferred->init(iWidth, iHeight, _pMsaaDepth, _pPick);// Yeah I don't know if the "pick" here will work
        _pMsaaForward = std::make_shared<ForwardFramebuffer>(Gu::getContext(), iWidth, iHeight, _bMsaaEnabled, _nMsaaSamples, _vClear);
    ...
    if(StringUtil::isNotEmpty(strEnvTexturePath)){
        _pEnvTex = Gu::getContext()->getTexCache()->getOrLoad(strEnvTexturePath, false, true, true);
    }
}

void RenderPipe::saveScreenshot() {
    if (Gu::getContext()->getFingers()->keyPress(SDL_SCANCODE_F9)) {
        if (Gu::getContext()->getFingers()->shiftHeld()) {
...
void RenderPipe::copyMsaaSamples(std::shared_ptr<FramebufferBase> msaa, std::shared_ptr<FramebufferBase> blitted) {
    ...
    if (_bMsaaEnabled)
    {
        Gu::getContext()->glBindRenderbuffer(GL_RENDERBUFFER, NULL);
        Gu::getContext()->chkErrDbg();

        Gu::getContext()->glBindFramebuffer(GL_READ_FRAMEBUFFER, msaa->getGlId());
        Gu::getContext()->chkErrDbg();
        Gu::getContext()->glBindFramebuffer(GL_DRAW_FRAMEBUFFER, blitted->getGlId());
        Gu::getContext()->chkErrDbg();
        ...
        for (std::shared_ptr<RenderTarget> inf : msaa->getTargets())
        {
            ...
            //GL_DEPTH_BUFFER_BIT;
            Gu::getContext()->glBlitFramebuffer(0, 0, _iLastWidth, _iLastHeight, 0, 0, _iLastWidth, _iLastHeight, bitMask, blendMode);
            Gu::getContext()->chkErrDbg();
        }
        Gu::getContext()->chkErrDbg();
...
void RenderPipe::renderShadows() {
    Gu::getContext()->getLightManager()->update(_pShadowBoxFboMaster, _pShadowFrustumMaster);
    
    //Force refresh teh viewport.
    Gu::getContext()->getCamera()->getViewport()->updateChanged(true);
}
...
void RenderPipe::blitDeferredRender() {
    ...
    Gu::getContext()->getShaderMaker()->shaderBound(nullptr);//Unbind and reset shader.
    Gu::getContext()->glBindFramebuffer(GL_FRAMEBUFFER, _pMsaaForward->getGlId());
    Gu::getContext()->chkErrDbg();
    Gu::getContext()->glBindRenderbuffer(GL_RENDERBUFFER, 0);
    Gu::getContext()->chkErrDbg();
    ...
    _pDeferredShader->beginRaster();
    {
        ...
        {
            Gu::getContext()->glActiveTexture(inf->getTextureChannel());
            Gu::getContext()->chkErrDbg();
...
void RenderPipe::setShadowUf() {
    ...
    for (int iShadowBox = 0; iShadowBox < iNumGpuShadowBoxes; ++iShadowBox) {
        ...
        if (iTextureIndex < iMaxTexs) {
            GLuint texId = Gu::getContext()->getTexCache()->getDummy1x1TextureCube();;
            if (iShadowBox < Gu::getContext()->getLightManager()->getGpuShadowBoxes().size()) {
                std::shared_ptr<ShadowBox> pBox = Gu::getContext()->getLightManager()->getGpuShadowBoxes()[iShadowBox];
                if (pBox != nullptr) {
    ...
    for (int iShadowFrustum = 0; iShadowFrustum < iNumGpuShadowFrustums; ++iShadowFrustum) {
        iTextureIndex = _pMsaaDeferred->getNumNonDepthTargets() + iIndex;
        if(iTextureIndex < iMaxTexs) {
            GLuint texId = Gu::getContext()->getTexCache()->getDummy1x1Texture2D();
            if (iShadowFrustum < Gu::getContext()->getLightManager()->getGpuShadowFrustums().size()) {
                std::shared_ptr<ShadowFrustum> pFrust = Gu::getContext()->getLightManager()->getGpuShadowFrustums()[iShadowFrustum];
                if (pFrust != nullptr) {
...
DOFFbo::DOFFbo(int32_t w, int32_t h){
    //Create Quick FBO
    Gu::getContext()->glGenFramebuffers(1, &_uiDOFFboId);
    Gu::checkErrorsDbg();
    Gu::getContext()->glBindFramebuffer(GL_FRAMEBUFFER, _uiDOFFboId);
    Gu::checkErrorsDbg();
    Gu::getContext()->glFramebufferParameteri(GL_FRAMEBUFFER, GL_FRAMEBUFFER_DEFAULT_WIDTH, w);
    Gu::checkErrorsDbg();
    Gu::getContext()->glFramebufferParameteri(GL_FRAMEBUFFER, GL_FRAMEBUFFER_DEFAULT_HEIGHT, h);
    Gu::checkErrorsDbg();
...
DOFFbo::~DOFFbo(){
    Gu::getContext()->glDeleteFramebuffers(1, &_uiDOFFboId);
...
void RenderPipe::postProcessDOF() {
    ...
    if(Gu::getContext()->getRenderSettings()->getDOF()){

        std::shared_ptr<ShaderBase> pDofShader = Gu::getContext()->getShaderMaker()->getDepthOfFieldShader();
        std::shared_ptr<CameraNode> pCam = Gu::getContext()->getCamera();
        ...
        pDofShader->beginRaster();
        {
            ...
            //Camera params
            //pDofShader->setUf("_fFocalDepth", (GLvoid*)&Gu::getContext()->getLightManager()->getDeferredParams()->_fFocalDepth);
            pDofShader->setUf("_fFocalRange", (GLvoid*)&Gu::getContext()->getLightManager()->getDeferredParams()->_fFocalRange);
            //View pos
            pDofShader->setUf("_ufViewPos", (void*)&pos);

            //Draw Round 1
            Gu::getContext()->glBindFramebuffer(GL_DRAW_FRAMEBUFFER, _pDOFFbo->_uiDOFFboId);
            Gu::getContext()->glFramebufferTexture2D(GL_DRAW_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, _pDOFFbo->_uiTexId0, 0);
            GLint horiz = 0;
            ...
            Gu::getContext()->glBindFramebuffer(GL_DRAW_FRAMEBUFFER, _pDOFFbo->_uiDOFFboId);
            Gu::getContext()->glFramebufferTexture2D(GL_DRAW_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, rtColor->getGlTexId(), 0);
...
void RenderPipe::renderScene( PipeBits pipeBits, std::shared_ptr<Drawable> toDraw){
    //Re-Init
    if (Gu::getContext()->getCamera() == nullptr) {
        BroLogError("Camera was not set for renderScene");
        return;
    }
    if (Gu::getContext()->getCamera()->getViewport() == nullptr) {
        BroLogError("Camera Viewport was not set for renderScene");
        return;
    }
    std::shared_ptr<Viewport> pv = Gu::getContext()->getCamera()->getViewport();
