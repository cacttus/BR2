...
void UiElement::computeQuads(float final_r, float final_l, float final_t, float final_b) {
    ...
    Gu::guiQuad2d(_b2RasterQuad, Gu::getContext()->getCamera()->getViewport());
}
...
void UiElement::drawBoundBox(std::shared_ptr<UtilMeshInline2d> mi, vec4& color, bool bPickedOnly) {
    if (bPickedOnly && (_iPickedFrameId != Gu::getContext()->getFpsMeter()->getFrameNumber())) {
...
void UiElement::enableDrag(UiDragInfo::DragFunc func) {
    ...
    addEvent(UiEventId::e::Mouse_Lmb_Press, UiEventFunc::create(
        ...
        if (std::shared_ptr<UiElement> w = winw.lock()) {
            ...
            w->_pDragInfo->_vDragStart = Gu::getContext()->getFingers()->getMousePos();
...
void UiElement::drawDebug() {
    //ONLY CALL ON Gui2d!!
    //Debug Rendering
    if (Gu::getContext()->getRenderSettings()->getDebug()->getShowGuiBoxesAndDisableClipping()) {
        //draws all children
        Gu::getContext()->setLineWidth(1.0f);
        std::shared_ptr<UtilMeshInline2d> mi = std::make_shared<UtilMeshInline2d>(Gu::getContext());
    ...
    if (Gu::getContext()->getRenderSettings()->getDebug()->getPickGui()) {
        //draws all children
        Gu::getContext()->setLineWidth(2.0f);
        std::shared_ptr<UtilMeshInline2d> mi = std::make_shared<UtilMeshInline2d>(Gu::getContext());
...
std::shared_ptr<UiImage> UiImage::create(std::shared_ptr<UiTex> tex, UiImageSizeMode::e eSizeX, UiImageSizeMode::e eSizeY, float fWidthPx, float fHeightPx) {
    ...
    pImg->_iPickId = Gu::getContext()->getPicker()->genPickId();
    return pImg;
}
UiImage::UiImage() {
}
...
bool UiImage::pick(std::shared_ptr<Fingers> fingers) {
    if (getLayoutVisible()) {
        if (getRenderVisible()) {
            if (_iPickId > 0) {
                uint32_t pixid = Gu::getContext()->getPicker()->getSelectedPixelId();
                if (pixid != 0) {
                    if (pixid == _iPickId) {
...
void UiDropdown::init() {
    ...
    Gu::getContext()->getGui()->addChild(_pListContainer);
    ...
    setPress([cont_w, dd_w, contW]() {
        ...
        if (cont && dd) {
            ...
            Gu::getContext()->getGui()->bringToFront(cont, true);
...
void UiToolbar::init() {
    ...
    //Set some default position stuff so we can see it.
    top() = "0px";
    left() = uDim(Gu::getContext()->getGui()->getDesignSize().getWidth() * 0.1f, UiDimUnit::e::Pixel);
    width() = "auto";//uDim(Gu::getContext()->getGui()->getDesignSize().getWidth() * 0.8f, UiDimUnit::e::Pixel);
    height() = "auto";//uDim(Gu::getContext()->getGui()->getDesignSize().getHeight() * 0.1f, UiDimUnit::e::Pixel);
...
void Gui2d::init() {
    ...
    right() = _designSize.getWidth() - 1;//Gu::getContext()->getViewport()->getWidth();
    bottom() = _designSize.getHeight() - 1;//Gu::getContext()->getViewport()->getHeight();
    top() = 0;
    left() = 0;
    minWidth() = maxWidth() = _designSize.getWidth() - 1;//right();
    minHeight() = maxHeight() = _designSize.getHeight() - 1;//bottom();
    setName("Gui2d");
    _pTex = std::make_shared<MegaTex>(Gu::getContext());
...
void Gui2d::update(std::shared_ptr<Fingers> pFingers) {
   ...
   // if (Gu::getContext()->getFpsMeter()->frameMod(2)) {
...
void Gui2d::drawForward(RenderParams& rp, Box2f&  b2ClipRect) {
    rp.setShader(Gu::getContext()->getShaderMaker()->getGuiShader());
...
void Gui2d::setCursor(std::shared_ptr<UiCursor> c) {
    ...
    //updateCursorPos(Gu::getContext()->getFingers()->getMousePos());
}
