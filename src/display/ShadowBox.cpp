...
void ShadowBox::update()
{
    ...
    if (!Gu::getContext()->getFpsMeter()->frameMod(1 + _iShadowBoxId)) {
        _bMustUpdate = false;
        return;
    }
...
void ShadowBox::copyAndBlendToShadowMap(std::shared_ptr<ShadowBox> pBox)
{
    ...
    if (pBox->getGlTexId() != 0) {
        if (Gu::getContext()->getRenderSettings()->getSmoothShadows()) {
            std::shared_ptr<ShaderBase> pDofShader = Gu::getContext()->getShaderMaker()->getSmoothGenShader();
...
void ShadowBox::updateScreenQuad()
{
    ...
    //    (int)Gu::getContext()->getViewport()->getWidth(),
    //    (int)Gu::getContext()->getViewport()->getHeight()
    //);
    //Quad2f c = Gu::getContext()->getViewport()->getClientQuad();//DirectCast(Gu::GetRenderManager(), GLRenderSystem*)->getViewport()->getClientQuad();
