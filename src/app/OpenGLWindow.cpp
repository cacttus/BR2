...
namespace Game {
///////////////////////////////////////////////////////////////////
OpenGLWindow::OpenGLWindow(t_string windowTitle, bool is_main) : GraphicsWindow(is_main) {
    ...
    for (int iProf = 0; iProf < c_iMax_Profs; ++iProf) {
        ...
        if (iDepth >= 24 && ver >= minGLVersion && ver >= minGLSubversion) {
            //Test to maek sure data copie
            _pSelectedProfile = std::make_shared<GLProfile>(&profs[iProf]);
            break;
        }
        else {
...
void OpenGLWindow::setWindowAndOpenGLFlags(GLProfile& prof) {
    //Attribs
...
void OpenGLWindow::getOpenGLVersion(int& ver, int& subver, int& shad_ver, int& shad_subver) {
    char* tmp;
    t_string glver;
    t_string glslver;
...
void OpenGLWindow::checkForOpenGlMinimumVersion(int required_version, int required_subversion) {
...
void OpenGLWindow::initGLContext() {
    _context = SDL_GL_CreateContext(getSDLWindow());
    if (!_context) {
        BroThrowException("SDL_GL_CreateContext() error" + SDL_GetError());
    }
}
void OpenGLWindow::makeCurrent() {
    SDL_GL_MakeCurrent(getSDLWindow(), _context);
}
...
void OpenGLWindow::createContext(std::shared_ptr<AppBase> app) {
...
void OpenGLWindow::loadCheckProc() {
