...
namespace GameHost
{
    ...
    public class GameServer
    {
        ...
        public bool? Connect(bool doHandshake = true, Action afterConnect=null)
        {
            ...
            IAsyncResult result = MySocket.BeginConnect(ip, Globals.Network.GameServerPort, ConnectedToServer, this);
        ...
        public void Disconnect(bool blnWaitForResult = false)
        {
            ...
            try
            {
                ...
                if (blnWaitForResult)
                    result.AsyncWaitHandle.WaitOne(Globals.Network.GameHostDisconnectAttemptTimeoutMs, true);
            }
        ...
        public void Send(string buf, int iTimeout = Globals.Network.SendAndRecvTimeoutMs)
        {
            _objPacketMaker.SendPacket(buf, iTimeout);
            _objCommandHistory.Add(buf.Substring(0, 8));
        }
        public string Recv(int iTimeout = Globals.Network.SendAndRecvTimeoutMs)
        {
        ...
        private void ConnectedToServer(IAsyncResult ar)
        {
            ...
            try
            {
                ...
                Logger.LogInfo("Connected to " + ag.ComputerName);
            ...
            catch (System.Net.Sockets.SocketException se)
            {
                if (se.SocketErrorCode == System.Net.Sockets.SocketError.ConnectionRefused)
                {
                    ...
                    Logger.LogInfo("  Agent " + ComputerName + " is not running.");
                }
                if (se.SocketErrorCode == System.Net.Sockets.SocketError.TimedOut)
                {
                    Logger.LogInfo("  Agent " + ComputerName + " timed out.");
                }
                lock (ag._objConnectLockObject)
                {
