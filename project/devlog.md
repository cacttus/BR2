...
* Started testing the scintillation technique in PBR

*3/20/20*

* Removed BVHCollectionParams.
...
* Added dvec3.
* Updated Bezier Path.
* Split Path into a spline component and a Node component.  Added CubicBezierSpline and Spline.  Need to fix the path movement internals.
...
* Added async logging back in (previous commit botched).
* Fixed logging bug where GU was getting called in the asynchronous loop.
* Fixed MegaTex to re-compile when it gets changed.  This makes creating the UI a LOT easier.
* Fixed UI
* Implemented Named Textures for easier debugging
* Added automatic file version to BinaryFile.
* Fixed error ambiguity when loading textures
* **We need to fix MtTex, and UiTexs texture attributes due to loading**
* Finished implementing Window-Centric InputManager 
	* Each window now has its own input.  No more Gu::getInputManager
* Added a "global" input manager to capture input from every window, note, mouse cursor position doesn't make sense in global input...'
* Fixed bugs with input in GraphicsApi
...
the physics manager is not found.  This is because it's trying to run its start code when it is not rooted tffo the window, or even the Scene.
